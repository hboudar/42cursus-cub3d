...
/*   Updated: 2024/09/26 16:11:31 by hboudar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 			break ;
// 		mlx_put_pixel(image, x, y, ft_pixel(rand() % 255,
...
void	render_wall(t_cube *cube, double angle)
{
    ...
    {
        mlx_put_pixel(cube->image, cube->ray_index, j, ft_pixel(cube->window.c[0], cube->window.c[1], cube->window.c[2], 0xFF)); // Sky blue
        j++;
    }
    ...
    {        (cube->player.direction == 'E') && (texture_y = (j - walltop_pixel) * cube->wall_1->height / wallstripheight);
        ...
        mlx_put_pixel(cube->image, cube->ray_index, j, color);
        j++;
    }
    j = wallbottom_pixel;
    while (j < HEIGHT)
    {
        mlx_put_pixel(cube->image, cube->ray_index, j, ft_pixel(cube->window.f[0], cube->window.f[1], cube->window.f[2], 0xFF)); // Floor white
        j++;
    }
}
